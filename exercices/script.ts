...
function sumDigitsInList<T extends number>(lista: T[]): number[] {
  ...
  for (let i: number = 0, n: number = lista.length; i < n; i++) {
    ...
    let numberSum = numberArr.reduce(
...
function averageGreaterThan<T extends number>(
  lista: T[],
  ref: number
): boolean {
  //valor que guarda a soma de todos os numeros do array
  let numberSum: number = lista.reduce((acc, cur) => acc + cur, 0);
...
function countWordsWithLetter<T extends string>(
  lista: T[],
  ref: string
): number {
  //setar contador
...
function groupByFirstLetter<T extends string>(
  lista: T[]
...
function getWordsWithVowelCount<T extends string>(
  lista: T[]
): { word: string; vowels: number }[] {
  let newArr: any = [];
  for (let i: number = 0, n: number = lista.length; i < n; i++) {
...
function emOrdemVogais<T extends string>(list: T[]): string[] {
  //função interna que retorna se as vogais de uma palavra estão ou não na ordem correta
  function isOrderedVowels(vowels: string[]): boolean {
...
function compareArrays<N, S>(list1: N[], list2: S[]) {
  function isNumberArr(arr: any[]) {
